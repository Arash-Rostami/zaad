...
import { ShieldCheck, MapPin, Maximize2, X, ChevronLeft, ChevronRight, ArrowUpRight } from "lucide-react";
...
export default function Showcase({
  ...
  const handleNextImage = () => {
    ...
    if (activeImageIndex < currentImagesCount - 1) {
      ...
      const currentItemIndex = COLLECTION_ITEMS.findIndex(item => item.id === selectedItem.id);
  ...
  const handlePrevImage = () => {
    if (activeImageIndex > 0) {
      ...
      const currentItemIndex = COLLECTION_ITEMS.findIndex(item => item.id === selectedItem.id);
      const prevItemIndex = (currentItemIndex - 1 + COLLECTION_ITEMS.length) % COLLECTION_ITEMS.length;
      const prevItem = COLLECTION_ITEMS[prevItemIndex];
      setSelectedItem(prevItem);
      setActiveImageIndex(prevItem.images.length - 1);
    }
  };
  return <section id="collection" className="py-24 md:py-36 bg-[#F4F1ED] px-6 sm:px-12 max-w-7xl mx-auto border-b border-[#1C1C1C]/10">
      <MaisonReveal variant="unveil" threshold={0.05}>
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 pb-6 border-b border-[#1C1C1C]/10 dark:border-white/10">
          ...
          <p className="text-xs font-mono text-[#5C5954] dark:text-[#97A5B8] max-w-xs mt-4 md:mt-0 leading-relaxed uppercase opacity-85">
            {isFarsi ? "سه‌گانه‌ای دقیق از آثار فیزیکی منحصربفرد که با خلوص کامل فضایی و تکیه بر اصالت مواد اولیه ساخته شده‌اند." : "A precise trilogy of physical objects designed with absolute spatial reduction and noble raw materials."}
          </p>
        </div>
        ...
        <div className="flex justify-center items-center space-x-6 sm:space-x-10 md:space-x-16 border-b border-[#1C1C1C]/10 dark:border-white/10 pb-0 mb-16 mx-auto w-full flex-wrap gap-y-4">
          ...
          return <button key={item.id} onClick={() => {
            ...
            setViewMode("editorial");
                ...
                <span className={`text-xs md:text-sm font-serif italic tracking-wider mb-2 transition-all duration-300 ${isActive ? "text-[#8E7A62] dark:text-[#CBB9A7] font-semibold scale-110" : "text-[#5C5954]/40 dark:text-white/30 group-hover:text-[#8E7A62]"}`}>
                  {item.number}
                </span>
                <span className={`text-base sm:text-lg md:text-xl font-mono tracking-[0.3em] uppercase transition-colors duration-300 ${isActive ? "text-[#1C1C1C] dark:text-[#EDECE8] font-semibold" : "text-[#5C5954]/50 dark:text-[#97A5B8]/50 group-hover:text-[#1C1C1C] dark:group-hover:text-white"}`}>
                  {itemTrans?.name || item.name}
                </span>
                {isActive && <motion.div layoutId="activeArchetypeTabLine" className="absolute bottom-0 left-0 right-0 h-[2.5px] bg-[#8E7A62] dark:bg-[#CBB9A7]" transition={{
              type: "spring",
              stiffness: 350,
              damping: 30
            }} />}
      ...
      <AnimatePresence mode="wait">
        ...
        ease: [0.16, 1, 0.3, 1]
          ...
          <MaisonReveal variant="scale-down-unveil" delay={0.15} className="lg:col-span-6 flex flex-col space-y-4 w-full">
            <div className="relative aspect-[4/5] bg-[#E8E4DF] border border-[#1C1C1C]/10 overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.03)] group select-none" style={{
            ...
            if (viewMode === "macro") setIsZooming(true);
          }} onTouchMove={handleTouchMove} onTouchEnd={() => {
            if (viewMode === "macro") setIsZooming(false);
          }} onTouchCancel={() => {
            if (viewMode === "macro") setIsZooming(false);
          }} onClick={() => {
            setIsEnlarged(true);
              ...
              <AnimatePresence mode="popLayout">
                <motion.img key={viewMode === "editorial" ? `${selectedItem.id}-${activeImageIndex}` : `${selectedItem.id}-macro`} initial={{
                opacity: 0,
                scale: 1.025
              }} animate={{
                opacity: 1,
              ...
              }} src={viewMode === "editorial" ? selectedItem.images[activeImageIndex]?.url : selectedItem.macroUrl} alt={`${selectedItem.name} (${viewMode === "editorial" ? `Image ${activeImageIndex + 1}` : "macro"} view)`} className="object-cover w-full h-full pointer-events-none" referrerPolicy="no-referrer" />
              </AnimatePresence>
              ...
              <div className="absolute top-4 right-4 pointer-events-none transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] z-20" style={{
              ...
              {viewMode === "editorial" && <div className="absolute bottom-6 right-6 flex items-center space-x-1.5 bg-white/95 backdrop-blur-sm border border-[#1C1C1C]/10 px-3 py-1.5 rounded-full font-mono text-[8px] tracking-wider text-[#1C1C1C] z-10 uppercase">
                  <span>{activeImageIndex + 1} / {selectedItem.images.length}</span>
                  <span className="text-[#8E7A62]">•</span>
                  <span>
                    {isFarsi ? selectedItem.images[activeImageIndex]?.orientation === "wide" ? "نمای افقی" : "نمای عمودی" : `${selectedItem.images[activeImageIndex]?.orientation || "PORTRAIT"} VIEW`}
                  </span>
                </div>}

              {viewMode === "macro" && <div className="absolute bottom-6 right-6 flex items-center space-x-2 bg-[#8E7A62] border border-[#8E7A62]/20 px-3 py-1.5 rounded-full font-mono text-[8px] tracking-widest text-white z-10 uppercase shadow-[0_4px_16px_rgba(0,0,0,0.15)] select-none">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-t border-[#1C1C1C]/10 pt-4">
              ...
              <div className="flex items-center space-x-2 w-full sm:w-auto justify-end">
                <MaisonButton variant={viewMode === "editorial" ? "pill-dark" : "pill-light"} onClick={() => setViewMode("editorial")} className="flex-1 sm:flex-initial text-center justify-center">
                  EDITORIAL VIEW
                </MaisonButton>
                <MaisonButton variant={viewMode === "macro" ? "pill-dark" : "pill-light"} onClick={() => setViewMode("macro")} className="flex-1 sm:flex-initial text-center justify-center">
                  MACRO TEXTURE
                </MaisonButton>
              </div>
            </div>
          ...
          <MaisonReveal variant="slide-up-royal" delay={0.3} className="lg:col-span-6 flex flex-col w-full text-left rtl:text-right">
            <div className="flex items-center justify-between mb-4 border-b border-[#1C1C1C]/10 pb-4">
              <span className="text-xs font-mono text-[#8E7A62] font-semibold tracking-widest">
                {selectedItem.number} / {isFarsi ? "مجموعه آرشیو" : "ARCHIVE COLLECTION"}
              </span>
            ...
            <div className="bg-[#E8E4DF]/60 p-6 border border-[#1C1C1C]/10 mb-8 rounded-xl">
              <h4 className="text-[10px] font-mono tracking-widest uppercase text-[#8E7A62] mb-2">
                {isFarsi ? "تک‌نگاری و ریشه‌های مادی اثر" : "MATERIAL MONOGRAPH"}
              </h4>
            ...
            <div className="border-t border-b border-[#1C1C1C]/10 dark:border-white/10 mb-8 overflow-hidden animate-none">
              <button type="button" onClick={() => setIsSpecsExpanded(!isSpecsExpanded)} className="w-full py-5 flex items-center justify-between group cursor-pointer focus:outline-none">
                <div className="flex items-center space-x-3 text-left rtl:text-right">
                  <span className="text-[10px] sm:text-xs font-mono tracking-[0.2em] text-[#8E7A62] dark:text-[#CBB9A7] font-semibold uppercase">
                    {isFarsi ? "مشخصات فنی و داده‌های اصلی اتلیه" : "STUDIO LOOKBOOK & ESSENTIAL DATA"}
                  </span>
                  <span className="text-[9px] font-mono text-[#5C5954] dark:text-[#97A5B8] opacity-65 group-hover:opacity-100 transition-opacity">
                    ({isSpecsExpanded ? isFarsi ? "بستن" : "COLLAPSE" : isFarsi ? "نمایش جزییات" : "SHOW STATS OVERVIEW"})
                  </span>
                </div>
                {/* Minimalist interactive indicators */}
                <div className="relative w-4 h-4 flex items-center justify-center">
                  <motion.div className="absolute w-3 h-[1px] bg-[#1C1C1C] dark:bg-[#EDECE8]" animate={{
              ...
              <AnimatePresence initial={false}>
                {isSpecsExpanded && <motion.div initial={{
                height: 0,
                opacity: 0
              }} animate={{
                height: "auto",
                ...
                ease: [0.16, 1, 0.3, 1]
                    ...
                    <div className="pt-2 pb-6 border-t border-[#1C1C1C]/10 dark:border-white/10 mt-1">
                      <div className="grid grid-cols-2 gap-y-5 gap-x-8 text-xs pb-4">
                        ...
                        <div className="space-y-1">
                          <span className="text-[9px] font-mono tracking-widest text-[#5C5954] dark:text-[#97A5B8] block uppercase">
                            {isFarsi ? "مدت زمان تحویل متغیر" : "Est. Curated Delivery"}
                          </span>
                          <span className="text-[#1C1C1C] dark:text-[#EDECE8] font-light text-xs sm:text-sm block leading-relaxed">
                        ...
                        <div className="col-span-2 pt-2">
                          <span className="text-[9px] font-mono tracking-widest text-[#5C5954] dark:text-[#97A5B8] block uppercase mb-2">
                            {isFarsi ? "ترکیب مواد خام تشکیل دهنده" : "Primary Composition Materials"}
                          </span>
                          <div className="flex flex-wrap gap-1.5">
                            {(getItemTranslations(selectedItem.id)?.materials || selectedItem.materials).map((mat, i) => <span key={i} className="text-[9px] bg-white/40 dark:bg-white/5 border border-[#1C1C1C]/10 dark:border-white/10 px-3 py-1 text-[#1C1C1C] dark:text-[#EDECE8] uppercase font-mono tracking-wide rounded-full">
                                {mat}
                              </span>)}
                          </div>
                        </div>
                      ...
                      <div className="mt-6 pt-4 border-t border-[#1C1C1C]/10 dark:border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <span className="text-[9px] font-mono text-[#5C5954] dark:text-[#97A5B8] leading-relaxed max-w-sm uppercase text-left rtl:text-right">
                          {isFarsi ? "توضیحات کامل به همراه نقشه‌ها و دستورالعمل‌های دو زبانه و جزییات لوازم خانگی Gaggenau در دفترچه جامع تدارکات وجود دارد." : "Complete technical descriptions, bilingual poetry logs, & Gaggenau appliance details exist in our dedicated master catalogue."}
                        </span>
                        <button type="button" onClick={() => onViewDetails(selectedItem)} className="font-mono text-[9px] tracking-[0.2em] bg-[#8E7A62] text-white hover:bg-[#1C1C1C] dark:hover:bg-white dark:hover:text-[#1C1C1C] py-2.5 px-5 rounded-full uppercase font-medium flex items-center space-x-2 rtl:space-x-reverse transition-all duration-300 focus:outline-none cursor-pointer hover:shadow-md">
                          <span>{isFarsi ? "مشاهده پرونده سفارشی اثر" : "REVEAL BESPOKE DOSSIER"}</span>
            ...
            <div className="flex items-center space-x-2 mt-4 text-[#5C5954]/70 text-[10px] font-mono tracking-wider justify-center sm:justify-start">
              ...
              <span>Complimentary insured airfreight with whiteglove placement globally.</span>
            </div>
          </MaisonReveal>
      ...
      <AnimatePresence>
        ...
        backgroundColor: "var(--bg-card-95)"
          ...
          cursor: lightboxScale > 1 ? "crosshair" : "zoom-in"
            ...
            }} onLoad={() => setIsLightboxLoading(false)} src={viewMode === "editorial" ? selectedItem.images[activeImageIndex]?.url : selectedItem.macroUrl} alt={selectedItem.name} style={{
          ...
          ease: [0.16, 1, 0.3, 1]
            ...
            pointerEvents: isZoomControllerHovered ? "auto" : "none"
                ...
                <div className="flex items-center space-x-4">
                  {/* PREV button */}
                  {viewMode === "editorial" ? <button onClick={e => {
                e.stopPropagation();
                ...
                setIsLightboxLoading(true);
                    ...
                    </button> : <div className="w-6" /> // spacer
              }
          ...
          ease: [0.16, 1, 0.3, 1]
              ...
              <div className="flex flex-col items-center md:items-start text-center md:text-left">
                <div className="flex items-center space-x-2.5">
                  ...
                  <span className="text-[var(--text-secondary)] opacity-30">•</span>
                  <span className="text-[var(--text-secondary)] opacity-80 text-[8.5px] font-mono tracking-[0.2em] uppercase">{viewMode} PERSPECTIVE</span>
                </div>
