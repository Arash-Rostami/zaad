...
export default function Header({
  ...
  return <header className="fixed top-0 left-0 w-full z-50 bg-white/10 hover:bg-white/20 backdrop-blur-[6px] border-b border-[#1C1C1C]/5 transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-12 py-3.5 sm:py-4 flex items-center justify-between relative">
        ...
        <div className="flex items-center">
          <MaisonButton onClick={() => setMenuOpen(!menuOpen)} variant="solid" className="!px-3.5 sm:!px-5 h-8 sm:h-10 shadow-sm relative z-50 !rounded-full !text-[8.5px] sm:!text-[10px] !tracking-[0.15em] sm:!tracking-[0.25em] flex items-center justify-center font-sans">
            {menuOpen ? isFarsi ? "بستن" : "Close" : isFarsi ? "کاتالوگ" : "Browse"}
          </MaisonButton>
        </div>
        ...
        <div className="absolute left-1/2 -translate-x-1/2 z-10 flex items-center justify-center">
          ...
          setMenuOpen(false);
          animateScrollToTop(1400); // 1.4s luxury cinematic return to top
        ...
        }} variant="outline" className="!px-3 sm:!px-5 h-8 sm:h-10 !rounded-full !text-[8.5px] sm:!text-[10px] !tracking-[0.15em] sm:!tracking-[0.2em] flex items-center justify-center font-sans">
      ...
      <AnimatePresence>
        ...
        originY: 0
      }} className="absolute top-full left-0 w-full bg-gradient-to-b from-[#FAF8F5]/98 via-[#F5F2EC]/98 to-[#EFECE6]/99 dark:from-[#090A09]/98 dark:via-[#0F0F0E]/98 dark:to-[#060706]/99 mid:from-[#0E1116]/98 mid:via-[#13171F]/98 mid:to-[#0A0C10]/99 backdrop-blur-3xl border-b border-[#8E7A62]/20 dark:border-white/10 mid:border-white/10 shadow-[0_50px_110px_rgba(0,0,0,0.18)] z-40 overflow-hidden">
            {/* Fine Architectural Grid Pattern Underlay for elite luxury technical aesthetic */}
            <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-20 dark:opacity-10 mix-blend-overlay">
              <svg className="w-full h-full stroke-[#8E7A62]/25 dark:stroke-white/15" xmlns="http://www.w3.org/2000/svg">
                <defs>
                  <pattern id="luxMenuGrid" width="32" height="32" patternUnits="userSpaceOnUse">
                    <path d="M 32 0 L 0 0 0 32" fill="none" strokeWidth="0.5" />
                  </pattern>
                </defs>
                <rect width="100%" height="100%" fill="url(#luxMenuGrid)" />
              </svg>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-6 sm:px-12 py-5 sm:py-6 flex flex-col justify-between relative z-10 max-h-[85vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-5">
                ...
                animateScrollToTop(1400);
                    ...
                    <div className="flex justify-between items-center w-full">
                      <span className="text-xs font-serif text-[#1C1C1C] dark:text-white mid:text-[#EBEFF5] font-semibold group-hover:translate-x-0.5 rtl:group-hover:-translate-x-0.5 transition-transform duration-300">
                        {isFarsi ? "۱. نمایشگاه همه‌جانبه‌ی زاد" : "01 / Living Showroom"}
                      </span>
                    ...
                    <span className="text-[9.5px] text-[#5C5954] dark:text-[#9C9588] mid:text-[#97A5B8] font-light mt-0.5 leading-none">
                      {isFarsi ? "جزیره‌های آشپزخانه و مبلمان تراورتن مونوکلتیک" : "Monolithic installations & specimen explorer"}
                    </span>
                  </button>
                ...
                setMenuOpen(false);
                    ...
                    <div className="flex justify-between items-center w-full">
                      <span className="text-xs font-serif text-[#1C1C1C] dark:text-white mid:text-[#EBEFF5] font-semibold group-hover:translate-x-0.5 rtl:group-hover:-translate-x-0.5 transition-transform duration-300">
                        {isFarsi ? "۲. نقشه‌های فنی آتلیه" : "02 / Atelier Blueprints"}
                      </span>
                      <ChevronRight className="w-3.5 h-3.5 text-[#8E7A62] mid:text-[#F2D299] group-hover:translate-x-0.5 rtl:group-hover:-translate-x-0.5 rtl:rotate-180 transition-transform duration-300" />
                    </div>
                    <span className="text-[9.5px] text-[#5C5954] dark:text-[#9C9588] mid:text-[#97A5B8] font-light mt-0.5 leading-none">
                      {isFarsi ? "کدهای مادی، فواصل هندسی و هندسه‌ی رنگ‌ها" : "Visual alignment & technical guidelines"}
                    </span>
                  </button>
                </div>
              ...
              }].map(target => <button key={target.key} onClick={() => {
                setActiveTab("showroom");
                onSelectProduct(null);
                setMenuOpen(false);
                setTimeout(() => {
                  ...
                  return <button key={item.id} onClick={() => {
                    ...
                    setMenuOpen(false);
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-[9px] font-mono text-[#8E7A62] dark:text-[#CBB9A7] mid:text-[#F2D299] mt-0.5 font-semibold leading-none">{item.number}</span>
                          </div>
              ...
              <div className="md:col-span-12 border-t border-[#1C1C1C]/10 dark:border-white/10 mt-5 pt-5 flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center space-x-3.5 rtl:space-x-reverse font-mono text-[8.5px] tracking-widest text-[#8E7A62]/80 dark:text-[#CBB9A7]/80 mid:text-[#F2D299]/80 uppercase">
                  ...
                  <span>{isFarsi ? "نسخه دیجیتال ۴.۰" : "DIGITAL EDITION v4.0"}</span>
                  <span className="opacity-30">•</span>
                  <span>{isFarsi ? "آرشیو کارگاه شماره ۲" : "ATELIER SPECIMEN INDEX N°02"}</span>
                </div>
                
                {/* Right controls */}
                <div className="flex items-center space-x-4">
                  <div className="flex flex-wrap items-center gap-3 bg-[#EAE5DF]/60 dark:bg-[#121211]/90 mid:bg-[#1C212B]/90 backdrop-blur-md border border-[#8E7A62]/15 dark:border-white/10 mid:border-white/10 px-3 py-1.5 rounded-full shadow-[0_5px_15px_rgba(0,0,0,0.03)] transition-all">
                    
                    {/* Language toggler */}
                    <div className="flex items-center relative rounded-full bg-[#1C1C1C]/5 dark:bg-white/5 mid:bg-black/25 p-0.5 font-mono text-[8px] tracking-widest h-7 w-20">
                      <button onClick={() => setLanguage("en")} className={`flex-1 text-center h-full rounded-full transition-all duration-300 relative z-10 uppercase text-[8px] font-semibold flex items-center justify-center ${language === "en" ? "text-white dark:text-[#111110] mid:text-[#1F242C] font-bold" : "text-[#5C5954] hover:text-[#1C1C1C] dark:text-[#9C9588] dark:hover:text-white mid:text-[#97A5B8] mid:hover:text-white"}`}>
                        {language === "en" && <motion.div layoutId="activeLanguageBlobInNavbar" className="absolute inset-0 bg-[#1C1C1C] dark:bg-[#EDECE8] mid:bg-[#F2D299] rounded-full z-[-1]" transition={{
                      type: "spring",
                      stiffness: 380,
                      damping: 25
                    }} />}
                      ...
                      <button onClick={() => setLanguage("fa")} className={`flex-1 text-center h-full rounded-full transition-all duration-300 relative z-10 text-[8px] font-semibold flex items-center justify-center ${language === "fa" ? "text-white dark:text-[#111110] mid:text-[#1F242C] font-bold" : "text-[#5C5954] hover:text-[#1C1C1C] dark:text-[#9C9588] dark:hover:text-white mid:text-[#97A5B8] mid:hover:text-white"}`}>
                        {language === "fa" && <motion.div layoutId="activeLanguageBlobInNavbar" className="absolute inset-0 bg-[#1C1C1C] dark:bg-[#EDECE8] mid:bg-[#F2D299] rounded-full z-[-1]" transition={{
                      type: "spring",
                      stiffness: 380,
                      damping: 25
                    }} />}
                    ...
                    <div className="flex items-center space-x-1.5 rtl:space-x-reverse pr-0.5">
                      ...
                      <div className="flex items-center relative rounded-full bg-[#1C1C1C]/5 dark:bg-white/5 mid:bg-black/25 p-0.5 h-7">
                        {["light", "mid", "dark"].map(mode => {
                      const isActive = themeMode === mode;
                      const label = mode === "light" ? "LMN" : mode === "mid" ? "ARA" : "UMB";
                      return <button key={mode} onClick={() => handleThemeChange(mode)} className={`px-2 h-full text-[8.5px] font-semibold font-mono tracking-widest rounded-full transition-all duration-300 relative z-10 flex items-center justify-center ${isActive ? "text-white dark:text-[#111110] mid:text-[#1F242C] font-bold" : "text-[#5C5954]/70 hover:text-[#1C1C1C] dark:text-[#9C9588]/70 dark:hover:text-white mid:text-[#97A5B8]/70 mid:hover:text-white"}`}>
                              {isActive && <motion.div layoutId="activeThemeBlobInNavbar" className="absolute inset-0 bg-[#1C1C1C] dark:bg-[#EDECE8] mid:bg-[#F2D299] rounded-full z-[-1]" transition={{
                          type: "spring",
                          stiffness: 380,
                          damping: 25
                        }} />}
