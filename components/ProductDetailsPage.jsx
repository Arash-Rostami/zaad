...
import { ArrowLeft, MapPin, Scale, Calendar, Layers, Sparkles, Cpu, ChevronRight, ShieldCheck, ChevronLeft, X, Maximize2 } from "lucide-react";
...
export default function ProductDetailsPage({
  ...
  return <div className="bg-[#F4F1ED] text-[#1C1C1C] min-h-screen py-10 md:py-16 px-4 sm:px-8 md:px-12 max-w-7xl mx-auto border-b border-[#1C1C1C]/10 transition-colors duration-1050">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#1C1C1C]/10 dark:border-white/10 pb-6 mb-8 sm:mb-12">
        <button onClick={onBack} className="group flex items-center space-x-3 rtl:space-x-reverse text-xs font-mono tracking-widest text-[#5C5954] dark:text-[#97A5B8] hover:text-[#1C1C1C] dark:hover:text-white transition-colors duration-300 focus:outline-none cursor-pointer">
          <motion.span className="inline-block rtl:rotate-180" animate={{
          x: [0, -4, 0]
        }} transition={{
          repeat: Infinity,
          duration: 1.5,
          ...
          <span>{isFarsi ? "بازگشت به صفحه نمایشگاه گالری" : "RETURN TO COLLAGE SHOWROOM"}</span>
        </button>
        
        <div className="flex items-center space-x-2 rtl:space-x-reverse text-[10px] font-mono tracking-widest uppercase">
          <span className="text-[#5C5954] dark:text-[#97A5B8] opacity-60">{isFarsi ? "آرشیو کارگاه زاد" : "ATELIER ARCHIVE"}</span>
          <span className="text-[#8E7A62] dark:text-[#CBB9A7] font-semibold">{item.number}</span>
          <span className="text-[#5C5954] dark:text-[#97A5B8] opacity-60">/</span>
          <span className="text-[#1C1C1C] dark:text-[#EDECE8] font-medium">{itemTrans?.name || item.name} {isFarsi ? "پرونده فنی و زیبایی" : "DIRECTORY"}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start mb-16 md:mb-24">
        ...
        <div className="lg:col-span-7 flex flex-col space-y-4 w-full">
          <div className="relative aspect-[16/10] sm:aspect-[16/9] w-full bg-[#E8E4DF] border border-[#1C1C1C]/10 dark:border-white/10 overflow-hidden shadow-xl rounded-sm cursor-zoom-in group" onMouseEnter={() => setIsHoveredOverImage(true)} onMouseLeave={() => setIsHoveredOverImage(false)} onClick={() => setIsEnlarged(true)}>
            
            {/* Carousel display with nice blur cross-fade */}
            <AnimatePresence mode="wait">
              <motion.img key={activeImageIndex} initial={{
              opacity: 0,
              scale: 1.015
            }} animate={{
              opacity: 1,
              ...
              duration: 0.65,
              ease: [0.16, 1, 0.3, 1]
            }} src={item.images[activeImageIndex]?.url} alt={item.images[activeImageIndex]?.caption || item.name} className="w-full h-full object-cover transition-transform duration-[1200ms] group-hover:scale-[1.03]" referrerPolicy="no-referrer" />
            </AnimatePresence>
            ...
            <div className="absolute bottom-4 left-4 bg-black/50 backdrop-blur-md px-3 py-1.5 rounded-lg border border-white/10 font-mono text-[9px] text-white/95 tracking-widest max-w-[85%] truncate z-10">
              VIEW {activeImageIndex + 1} OF {item.images.length}: {item.images[activeImageIndex]?.caption || `${item.name} layout`}
            </div>

            {/* High-Fidelity Corner Maximizer Signifier */}
            <div className="absolute top-4 right-4 pointer-events-none transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] z-15" style={{
          ...
          <div className="grid grid-cols-3 gap-3">
            {item.images.map((img, idx) => <button key={idx} onClick={() => setActiveImageIndex(idx)} className={`relative aspect-[16/10] overflow-hidden border transition-all duration-500 rounded-sm cursor-pointer ${activeImageIndex === idx ? "border-[#8E7A62] dark:border-[#CBB9A7] ring-1 ring-[#8E7A62] dark:ring-[#CBB9A7]" : "border-[#1C1C1C]/10 dark:border-white/10 opacity-60 hover:opacity-90"}`}>
                <img src={img.url} alt={`Perspective thumbnail ${idx}`} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
              </button>)}
          </div>
        </div>
        ...
        <div className="lg:col-span-4 flex flex-col justify-start text-left rtl:text-right">
          <MaisonReveal variant="slide-up-royal">
            ...
            <p className="text-xs font-mono tracking-[0.15em] text-[#5C5954] dark:text-[#97A5B8] uppercase mb-6 pb-4 border-b border-[#1C1C1C]/10 dark:border-white/10">
              {isFarsi ? "معمار اثر" : "Curator"}: {item.designer} • {isFarsi ? "آرشیو کارگاه سال" : "Atelier"} {item.year} {isFarsi ? "" : "Release"}
            </p>
            ...
            <div className="flex flex-col sm:flex-row items-stretch gap-4">
              <MaisonButton variant="solid" onClick={() => onInquire(item)} className="flex-1 text-center justify-center font-semibold text-xs py-3.5 font-sans">
                {isFarsi ? "آغاز فرآیند تدارکات و سفارش" : "INITIATE PRIVATE CONCIERGE INQUIRY"}
              </MaisonButton>
              <MaisonButton variant="outline" onClick={onBack} className="text-center justify-center font-mono text-[10px] tracking-widest text-[#5C5954]">
                RETURN TO GRID
              </MaisonButton>
            </div>
          </MaisonReveal>
      ...
      {item.farsiStory && <MaisonReveal variant="unveil">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 items-stretch py-12 md:py-16 px-6 md:px-10 bg-[#E8E4DF]/20 dark:bg-black/15 border border-[#1C1C1C]/5 dark:border-white/5 rounded-2xl mb-16 md:mb-24 relative overflow-hidden">
            ...
            <div className="absolute right-6 top-6 opacity-[0.03] select-none text-[6rem] font-serif pr-4 leading-none" dir="rtl">زمین</div>
            
            {/* Farsi text block with heavy gold sidebar */}
            <div className="md:col-span-6 border-r-2 border-[#8E7A62] pr-6 flex flex-col justify-center text-right" dir="rtl">
              ...
              <div className="space-y-4">
                {item.farsiStory.paragraphs.map((p, idx) => <p key={idx} className="text-[12px] sm:text-[13px] text-[#5C5954] dark:text-[#97A5B8] leading-loose font-light antialiased">
                    {p}
                  </p>)}
              </div>
            </div>
      ...
      <div className="mb-20">
        <div className="flex justify-center border-b border-[#1C1C1C]/10 dark:border-white/10 mb-10 overflow-x-auto scrollbar-none">
          <div className="flex space-x-8 md:space-x-12 pb-px shrink-0">
            ...
            return <button key={tab.id} onClick={() => setActiveTab(tab.id)} className="group relative pb-4 flex items-center space-x-2 transition-all duration-300 cursor-pointer focus:outline-none shrink-0">
                  <IconComp className={`w-3.5 h-3.5 ${isTabActive ? "text-[#8E7A62] dark:text-[#CBB9A7]" : "text-[#5C5954]/50 group-hover:text-[#8E7A62]"}`} />
                  <span className={`text-[10.5px] font-mono tracking-[0.2em] uppercase transition-colors ${isTabActive ? "text-[#1C1C1C] dark:text-[#EDECE8] font-semibold" : "text-[#5C5954]/50 dark:text-[#97A5B8]/50 group-hover:text-[#1C1C1C] dark:group-hover:text-white"}`}>
                    {tab.label}
                  </span>
                  {isTabActive && <motion.div layoutId="activeCurationTabLine" className="absolute bottom-0 left-0 right-0 h-[2px] bg-[#8E7A62] dark:bg-[#CBB9A7]" transition={{
                type: "spring",
                stiffness: 350,
                damping: 30
              }} />}
        ...
        <AnimatePresence mode="wait">
          ...
          ease: [0.16, 1, 0.3, 1]
            ...
            {activeTab === "architecture" && <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
                
                {/* Island Split architecture specs */}
                {item.islandSpecs && <div className="bg-white/40 dark:bg-[#1C1C1B] p-6 sm:p-8 rounded-2xl border border-[#1C1C1C]/5 dark:border-white/5 flex flex-col justify-between">
                    <div>
                      <div className="flex items-center space-x-2 mb-4">
                        <span className="w-1.5 h-1.5 bg-[#8E7A62] rounded-full" />
                        <span className="text-[9.5px] font-mono tracking-[0.25em] text-[#8E7A62] uppercase font-semibold">CORE ISLAND GEOMETRIES</span>
                      </div>
                      ...
                      <div className="space-y-4">
                        {item.islandSpecs.partA && <div className="bg-[#E8E4DF]/30 dark:bg-white/5 p-4 rounded-xl">
                            <h5 className="font-mono text-[9px] font-bold text-[#8E7A62] dark:text-[#CBB9A7] uppercase mb-1.5">{item.islandSpecs.partA.title}</h5>
                            <ul className="list-disc list-inside space-y-1 text-[10.5px] text-[#5C5954] dark:text-[#97A5B8] leading-relaxed font-light">
                              {item.islandSpecs.partA.bullets.map((b, bIdx) => <li key={bIdx}>{b}</li>)}
                            </ul>
                          </div>}
                        {item.islandSpecs.partB && <div className="bg-[#E8E4DF]/30 dark:bg-white/5 p-4 rounded-xl">
                            <h5 className="font-mono text-[9px] font-bold text-[#8E7A62] dark:text-[#CBB9A7] uppercase mb-1.5">{item.islandSpecs.partB.title}</h5>
                            <ul className="list-disc list-inside space-y-1 text-[10.5px] text-[#5C5954] dark:text-[#97A5B8] leading-relaxed font-light">
                              {item.islandSpecs.partB.bullets.map((b, bIdx) => <li key={bIdx}>{b}</li>)}
                            </ul>
                          </div>}
                      </div>
                    </div>

                    {item.islandSpecs.listSpecs && <div className="mt-8 border-t border-[#1C1C1C]/10 dark:border-white/10 pt-6">
                        <span className="text-[8.5px] font-mono tracking-widest text-[#5C5954] block uppercase mb-3">CONSTRUCTOR MANUAL SHEETS</span>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[10px] font-mono">
                          {item.islandSpecs.listSpecs.map((s, sIdx) => <div key={sIdx} className="flex items-center space-x-1.5 p-2 bg-white/60 dark:bg-white/5 rounded border border-[#1C1C1C]/5">
                ...
                {item.tallUnits && <div className="bg-white/40 dark:bg-[#1C1C1B] p-6 sm:p-8 rounded-2xl border border-[#1C1C1C]/5 dark:border-white/5 flex flex-col justify-between">
                    <div>
                      <div className="flex items-center space-x-2 mb-4">
                        <span className="w-1.5 h-1.5 bg-[#8E7A62] rounded-full" />
                        <span className="text-[9.5px] font-mono tracking-[0.25em] text-[#8E7A62] uppercase font-semibold">TALL CORE ARCHITECTURES</span>
                      </div>
                      ...
                      {item.tallUnits.parts && <div className="space-y-2 mb-6 text-[11px]">
                          <span className="text-[8.5px] font-mono tracking-widest text-[#5C5954] dark:text-[#97A5B8] uppercase block mb-1">ATELIER TOWER ROW SCHEDULING</span>
                          <div className="grid grid-cols-1 gap-1.5 font-mono">
                            {item.tallUnits.parts.map((tower, tIdx) => <div key={tIdx} className="flex items-center justify-between p-2.5 bg-white/40 dark:bg-white/5 border border-[#1C1C1C]/5 rounded-lg">
                                <span className="text-[#8E7A62] font-bold text-[9px] shrink-0 uppercase">{tower.key}</span>
                                <span className="text-[#5C5954] dark:text-[#97A5B8] truncate text-right">{tower.name}</span>
                              </div>)}
                          </div>
                        </div>}

                      <div className="space-y-4">
                        {item.tallUnits.adjacentA && <div className="bg-[#E8E4DF]/30 dark:bg-white/5 p-4 rounded-xl">
                            <span className="font-mono text-[8px] tracking-widest text-[#8E7A62] dark:text-[#CBB9A7] block mb-1.5 uppercase">Ergonomics Plan (A-Adjacent)</span>
                            <p className="text-[10px] text-[#5C5954] dark:text-[#97A5B8] italic mb-2">{item.tallUnits.adjacentA.reason}</p>
                            <ul className="list-disc list-inside space-y-1 text-[10.5px] text-[#5C5954] dark:text-[#97A5B8] leading-relaxed font-light">
                              {item.tallUnits.adjacentA.bullets.map((b, bIdx) => <li key={bIdx}>{b}</li>)}
                            </ul>
                          </div>}
                        {item.tallUnits.adjacentB && <div className="bg-[#E8E4DF]/30 dark:bg-white/5 p-4 rounded-xl">
                            <span className="font-mono text-[8px] tracking-widest text-[#8E7A62] dark:text-[#CBB9A7] block mb-1.5 uppercase">Ergonomics Plan (B-Adjacent)</span>
                            <p className="text-[10px] text-[#5C5954] dark:text-[#97A5B8] italic mb-2">{item.tallUnits.adjacentB.reason}</p>
                            <ul className="list-disc list-inside space-y-1 text-[10.5px] text-[#5C5954] dark:text-[#97A5B8] leading-relaxed font-light">
                              {item.tallUnits.adjacentB.bullets.map((b, bIdx) => <li key={bIdx}>{b}</li>)}
                            </ul>
                          </div>}
                      </div>
                    </div>

                    {item.tallUnits.listSpecs && <div className="mt-8 border-t border-[#1C1C1C]/10 dark:border-white/10 pt-6">
                        <span className="text-[8.5px] font-mono tracking-widest text-[#5C5954] block uppercase mb-3">HOUSING STRUCTURAL COMPONENTS</span>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[10px] font-mono">
                          {item.tallUnits.listSpecs.map((s, sIdx) => <div key={sIdx} className="flex items-center space-x-1.5 p-2 bg-white/60 dark:bg-white/5 rounded border border-[#1C1C1C]/5">
                              <span className="text-[#8E7A62]">▪</span>
                              <span className="truncate">{s}</span>
                            </div>)}
                        </div>
            ...
            {activeTab === "appliances" && <div className="space-y-8">
                {/* Brand overview sheet */}
                {item.partners && <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 bg-white/40 dark:bg-[#1C1C1B] border border-[#1C1C1C]/5 p-6 rounded-2xl shadow-sm text-xs">
                    <div className="space-y-1">
                      <span className="text-[8px] font-mono text-[#5C5954] dark:text-[#97A5B8] block uppercase tracking-wider">CHASSIS TYPOLOGY</span>
                      <strong className="text-[11px] text-[#1C1C1C] dark:text-[#EDECE8] uppercase font-mono block">{item.partners.typology}</strong>
                    </div>
                    <div className="space-y-1">
                      <span className="text-[8px] font-mono text-[#5C5954] dark:text-[#97A5B8] block uppercase tracking-wider">HARDWARE CORE</span>
                      <strong className="text-[11px] text-[#1C1C1C] dark:text-[#EDECE8] uppercase font-mono block">{item.partners.hardware}</strong>
                    </div>
                    <div className="space-y-1">
                      <span className="text-[8px] font-mono text-[#5C5954] dark:text-[#97A5B8] block uppercase tracking-wider">INTEGRATED GLASSWARE</span>
                      <strong className="text-[11px] text-[#1C1C1C] dark:text-[#EDECE8] uppercase font-mono block">{item.partners.appliances}</strong>
                    </div>
                    <div className="space-y-1">
                      <span className="text-[8px] font-mono text-[#5C5954] dark:text-[#97A5B8] block uppercase tracking-wider">SMART ACC. & LED</span>
                      <strong className="text-[11px] text-[#1C1C1C] dark:text-[#EDECE8] uppercase font-mono block">{item.partners.accessories} • {item.partners.light}</strong>
                    </div>
                  </div>}

                {/* Gaggenau specifics details */}
                {item.appliancesDetail && <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {item.appliancesDetail.map((app, idx) => <div key={idx} className="bg-white/40 dark:bg-[#1C1C1B] p-5 sm:p-6 rounded-2xl border border-[#1C1C1C]/5 dark:border-white/5 flex flex-col justify-between">
                          ...
                          <div className="mt-5 pt-3 border-t border-[#1C1C1C]/5 dark:border-white/5 flex items-center justify-between">
                            <span className="text-[8px] font-mono text-[#5C5954] dark:text-[#97A5B8] uppercase">INTEGRATION RATING</span>
                            <span className="text-[9px] font-mono font-bold text-[#8E7A62]">GAGGENAU 200/400</span>
                          </div>
                        </div>)}
                    </div>
                  </div>}
                ...
                {item.accessoriesDetail && <div className="bg-white/40 dark:bg-[#1C1C1B] p-6 rounded-2xl border border-[#1C1C1C]/5 dark:border-white/5 space-y-4 mt-8">
                    <span className="text-[9px] font-mono tracking-widest text-[#8E7A62] block uppercase">KESSEBÖHMER BUILT-IN ACCESSORIES STRUCTURES</span>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {item.accessoriesDetail.map((acc, aIdx) => <div key={aIdx} className="space-y-2">
                          <h6 className="font-mono text-[11px] font-bold text-[#1C1C1C] dark:text-[#EDECE8] uppercase">{acc.name}</h6>
                          <div className="text-[10px] bg-[#E8E4DF]/30 dark:bg-white/5 p-3 rounded-lg border border-[#1C1C1C]/5">
                            <ul className="list-disc list-inside space-y-1 text-[#5C5954] dark:text-[#97A5B8]">
                              {acc.specs.map((s, sIdx) => <li key={sIdx} className="font-light">{s}</li>)}
                            </ul>
                          </div>
                        </div>)}
                    </div>
            ...
            {activeTab === "heritage" && <div className="bg-white/40 dark:bg-[#1C1C1B] rounded-2xl border border-[#1C1C1C]/5 p-6 sm:p-10 space-y-8">
                <div className="max-w-3xl">
                  <div className="flex items-center space-x-2 mb-4">
                    <Sparkles className="w-4 h-4 text-[#8E7A62] animate-pulse" />
                    <span className="text-[9px] font-mono tracking-widest text-[#8E7A62] uppercase font-semibold">CRAFT INTEGRITY SEAL</span>
                  </div>
                  ...
                  <p className="text-xs sm:text-sm text-[#5C5954] dark:text-[#97A5B8] leading-relaxed font-light mb-6">
                    Each ZAAD Atelier object represents a rigorous response of quiet luxury against dynamic trends. Organized symmetrically across heavy, grounded natural travertine/rapolano stone cores and durable eucalyptus veneers, the design relies strictly on authentic physical materials to establish spiritual calm within the domestic landscape.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-[#1C1C1C]/10 dark:border-white/10 text-xs">
                  <div className="space-y-1.5 p-5 bg-[#E8E4DF]/20 dark:bg-black/10 rounded-xl">
                    <h5 className="font-mono text-[10px] font-bold text-[#8E7A62] dark:text-[#CBB9A7] uppercase">THE RAW STONE CURATION</h5>
                  ...
                  <div className="space-y-1.5 p-5 bg-[#E8E4DF]/20 dark:bg-black/10 rounded-xl">
                    <h5 className="font-mono text-[10px] font-bold text-[#8E7A62] dark:text-[#CBB9A7] uppercase font-semibold">EUCALYPTUS VENEERS AND SADDLE LEATHER</h5>
                    <p className="text-[#5C5954] dark:text-[#97A5B8] leading-relaxed font-light">
      ...
      <div className="bg-[#E8E4DF] dark:bg-[#1C1C1B] rounded-2xl p-8 md:p-12 border border-[#1C1C1C]/10 dark:border-white/10 text-center relative overflow-hidden flex flex-col items-center justify-center">
        ...
        <p className="text-xs sm:text-sm text-[#5C5954] dark:text-[#97A5B8] max-w-xl font-light mb-8 leading-relaxed">
          Through ZAAD Atelier concierge services, each bespoke order is supervised from stone selection at the quarry to local installation by master technicians.
        </p>
        
        <div className="flex flex-col sm:flex-row items-center gap-4">
          <MaisonButton variant="solid" onClick={() => onInquire(item)} className="!px-8 !py-3.5 text-xs font-semibold">
            Initiate Curated Private Inquiry
          </MaisonButton>
          <MaisonButton variant="outline" onClick={onBack} className="text-[10px] font-mono tracking-widest text-[#5C5954] px-6 py-3">
      ...
      <AnimatePresence>
        ...
        duration: 0.75,
        ease: [0.16, 1, 0.3, 1]
      }} className="fixed inset-0 z-[120] flex flex-col items-center justify-center backdrop-blur-3xl p-6 md:p-16 cursor-zoom-out group/lightbox" style={{
        backgroundColor: "var(--bg-card-95)"
          ...
          cursor: lightboxScale > 1 ? "crosshair" : "zoom-in"
            ...
            nextImage();
              ...
              {isLightboxLoading && <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
                  <motion.div initial={{
              opacity: 0
            }} animate={{
          ...
          ease: [0.16, 1, 0.3, 1]
            ...
            const nextScale = Math.abs(lightboxScale - 1.0) < 0.1 ? 1.8 : Math.abs(lightboxScale - 1.8) < 0.2 ? 3.0 : 1.0;
            setLightboxScale(nextScale);
          }} className="w-8 h-8 rounded-full border border-[#1C1C1C]/10 dark:border-white/10 relative overflow-hidden flex items-center justify-center cursor-pointer shrink-0 z-10 bg-transparent" title="Cycle zoom level" animate={{
            ...
            scale: 0.92
              ...
              <motion.div className="flex items-center pl-3 pr-2 border-l border-[#1C1C1C]/10 dark:border-white/10 mr-1 shrink-0 overflow-hidden" animate={{
            ...
            pointerEvents: isZoomControllerHovered ? "auto" : "none"
                ...
                prevImage();
                setIsLightboxLoading(true);
              }} className="text-[#1C1C1C]/60 dark:text-white/60 hover:text-[#1C1C1C] dark:hover:text-white p-1 transition-colors cursor-pointer" title="Previous Image">
                  ...
                  return <button key={preset} onClick={e => {
                    e.stopPropagation();
                ...
                nextImage();
                setIsLightboxLoading(true);
              }} className="text-[#1C1C1C]/60 dark:text-white/60 hover:text-[#1C1C1C] dark:hover:text-white p-1 transition-colors cursor-pointer" title="Next Image">
            ...
            {lightboxScale > 1 && <span className="absolute bottom-20 left-1/2 -translate-x-1/2 text-[7.5px] tracking-[0.2em] font-mono text-[#8E7A62]/95 font-medium uppercase select-none animate-pulse pointer-events-none whitespace-nowrap z-50">
          ...
          duration: 0.7,
          ease: [0.16, 1, 0.3, 1]
        }} className="absolute bottom-6 left-6 right-6 md:left-12 md:right-12 flex flex-col md:flex-row items-center justify-between pointer-events-auto bg-transparent border-t border-[var(--border-color-10)] pt-4 w-auto max-w-5xl mx-auto z-40 gap-4" onClick={e => e.stopPropagation()}>
              <div className="flex flex-col items-center md:items-start text-center md:text-left">
                <div className="flex items-center space-x-2.5">
                  ...
                  <span className="text-[var(--text-secondary)] opacity-30">•</span>
                  <span className="text-[var(--text-secondary)] opacity-80 text-[8.5px] font-mono tracking-[0.2em] uppercase">ENLARGED PERSPECTIVE</span>
                </div>
              ...
              <div className="flex items-center space-x-6 w-full md:w-auto justify-between md:justify-end">
                ...
                <MaisonButton variant="outline" onClick={() => {
              setIsEnlarged(false);
              onInquire(item);
