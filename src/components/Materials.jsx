...
function Materials() {
    ...
    return (
        <section className="py-24 md:py-36 bg-surface-overlay px-6 sm:px-12 border-b border-ink/10 relative overflow-hidden text-left rtl:text-right">
            ...
            <span className="text-[10px] sm:text-xs font-mono tracking-[0.3em] text-accent font-semibold uppercase block mb-3">
              {t("materialArchaeology")}
                    ...
                    <span className="text-[10px] font-mono tracking-widest text-accent font-semibold block mb-1 uppercase">
                      {mat.category}
                    </span>
                      ...
                      {mat.name}
                                    ...
                                    <span className={`text-[11px] font-mono tracking-widest text-ink transition-transform duration-300 shrink-0 ${active?.id === mat.id ? "translate-x-1 font-semibold text-accent" : "opacity-40 group-hover:opacity-100"}`}>
                    {t("hexaSample")}
                  </span>
                                </MaisonButton>
                            ))}
                    ...
                    <MaisonReveal variant="scale-down-unveil" delay={0.2} className="lg:col-span-6">
                        <AnimatePresence mode="wait">
                            {active && (
                                ...
                                >
                                    <div className="relative aspect-video w-full overflow-hidden border border-ink/10 mb-6 rounded-xl">
                                        <img
                                            src={active.imageUrl}
                                        ...
                                        />
                                        <div className="absolute top-4 right-4 bg-ink text-canvas px-3 py-1 font-mono text-[9px] tracking-widest uppercase rounded-md">
                                            {t("macroPreview")}
                                        </div>
                                    </div>
